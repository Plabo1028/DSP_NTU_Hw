...
/*Read Big-5_xxxxx, and map zuyin to word*/
...
/* Seperate the input line with blank for a word. */
...
/* For a zhuyin, find all the possible words prefix with zhuyin. */
vector< set<string> > FindAllPossible(vector<string> InputLine,
                                      map<string, set<string> > mapping)
{
   ...
   for(int i=0 ; i<InputLine.size() ; i++){
      ...
      if(map_iter!=mapping.end()){      //Find a zhuyin.
         TmpSet = (map_iter->second);
      }else{
         TmpSet.insert(InputLine.at(i)); //Find none, so it is not zhuyin.
      }
...
/* Find the maximum probability for all possible bi-gram.*/
...
vector<string> Viterbi(vector< set<string> > AllNodes, Ngram &lm, Vocab &vocab)
{
   ...
   for(int i=1 ; i<AllNodes.size() ; i++){
      ...
      set<string> PsbWords = CopySet(AllNodes.at(i)); //All possible words at node_i
